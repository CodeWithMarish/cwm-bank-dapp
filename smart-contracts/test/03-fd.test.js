  ...
  : describe("FixedDeposit", () => {
      ...
      describe("Starting Test", () => {
        ...
        it("Check returns ", async () => {
          ...
          await expect(
            fd1.approveClaim(accounts[0].address)
          ).to.be.revertedWith("You are not employee");
          let tx_1 = await bank.addEmployee(accounts[1].address, 0);
          await tx_1.wait();
          let tx_2 = await bank.addEmployee(accounts[2].address, 0);
          await tx_2.wait();
          let fd2 = await fd.connect(accounts[2]);
          await expect(
            fd1.approveClaim(accounts[1].address)
          ).to.be.revertedWith(
            "canApproveClaim: Claim request does not exists"
          );

          let tx2 = await fd1.approveClaim(accounts[0].address);
          await tx2.wait();
          assert.equal((await fd.getFD(accounts[0].address)).status, 2);
          await expect(
            fd1.approveClaim(accounts[0].address)
          ).to.be.revertedWith("canApproveClaim: You have already approved");
          let tx2_1 = await fd2.approveClaim(accounts[0].address);
          await tx2_1.wait();

          assert.equal((await fd.getFD(accounts[0].address)).status, 3);
          assert.equal(
            (await fd.getFD(accounts[0].address)).claimConfirmations,
            2
          );
          assert.equal(
